...
import { HalalCert } from '@/lib/types';

export default function HalalSheetScreen() {
  const params = useLocalSearchParams<{ locationId: string }>();
  const locationId = String(params.locationId ?? '');
  const insets = useSafeAreaInsets();

  const { data: cert } = useQuery({
    queryKey: ['halal', locationId],
    queryFn: async () => {
      try {
        const { data } = await supabase
          ...
          .maybeSingle();
        return (data ?? null) as HalalCert | null;
      } catch {
        return null;
      }
    },
      ...
      >
        ...
        >
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
            ...
            <View>
              <Text style={{ fontSize: 13, color: '#6B7280', fontWeight: '600' }}>Certified Halal</Text>
        ...
        <ScrollView contentContainerStyle={{ padding: 20, gap: 18, paddingTop: 8 }}>
          ...
          >
            ...
            <Text style={{ fontSize: 12.5, color: '#6B7280', marginTop: 4, lineHeight: 18 }}>
              The certifying agency verifies that ingredients and preparation follow halal
              standards. Tap the certificate to view full details.
            </Text>
          </View>
        </ScrollView>
      </View>
