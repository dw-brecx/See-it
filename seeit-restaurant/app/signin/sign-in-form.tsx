...
export function SignInForm() {
  ...
  useEffect(() => {
    if (searchParams.get('error') === 'not_authorized') {
      setError("That account isn't authorized for the restaurant dashboard.");
    }
  }, [searchParams]);
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      const allowed = profile.role === 'restaurant_owner' || profile.role === 'admin';
      if (!allowed) {
        await supabase.auth.signOut();
        throw new Error(
          "That account isn't a restaurant owner. Sign up to create a restaurant account.",
        );
      }
